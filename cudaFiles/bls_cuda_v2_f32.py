...
def bls(gbls_inputs, time = np.array([0]), flux = np.array([0])):
    ...
    time_g  = cuda.to_device(time.astype(np.float32))
    flux_g  = cuda.to_device(flux.astype(np.float32))
    ...
    dconst    = np.zeros((8), dtype = np.float32)
    dconst[0] = Mstar
    dconst[1] = Rstar
    dconst[2] = G
    dconst[3] = Rsun
...
def compute_bls(freqs, power, jn1, jn2, time_g, flux_g, const_g, dconst_g):
    ...
    freqs_g = cuda.to_device(freqs.astype(np.float32))
    power_g = cuda.to_device(power.astype(np.float32))
...
def bls_kernel(time_g, flux_g, const_g, dconst_g, freqs_g, power_g, jn1_g, jn2_g):
    ...
    y   = cuda.shared.array(2048, dtype=np.float32)
    
    # Thread block reduction arrays
    best_p_shared = cuda.shared.array(128, dtype=np.float32)
